...
# Use a NullHandler by default so libraries using this module don't configure root logging
...
def handle_get(oid_str, ctx=None):
    ...
    try:
        # Generated handlers follow signature get_<name>(oid=None, ctx=None)
...
def handle_set(oid_str, value, ctx=None):
    ...
    if not _check_access(name, 'set', ctx):
        ...
        raise PermissionError('access denied')
    try:
        # Generated handlers follow signature set_<name>(oid, value, ctx=None)
